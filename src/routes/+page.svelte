		...
		<aside class="h-full grid grid-cols-1 grid-rows-[auto_1fr_1fr] gap-2 p-2 bg-[#111] min-h-0">
						...
						<li>
							...
							<ul>
								{#each nodesDesc[nodeId].stats as stat}
									<li class="text-sm text-[#7d7aad]">{stat}</li>
								{/each}
							</ul>
						</li>
					{/each}
			...
			<div class="min-h-0 grid grid-cols-1 grid-rows-[auto_auto_auto_1fr]">
				...
				<div class="flex flex-row justify-between">
					...
					<span
						>Selected:
						{selectedNodes.length} / {Object.entries(nodesDesc).filter(([_, v]) => v.stats.length)
							.length}
					</span>
				</div>
				<ul class="block min-h-0 overflow-y-auto">
		  ...
		  "
			  ...
			  "
							...
							{#each nodes[kind] as node}
								{#if !(hideSmall && node.id.startsWith('S'))}
								  ...
								  left: {node.x * imageEl.naturalWidth * scale -
													((baseNodeSize +
														node.id.startsWith('K') * 4 -
														node.id.startsWith('S') * 10) *
														scale) /
														2}px;
				...
				{#if tooltipContent != null}
					<div
						bind:this={tooltipEl}
						class="tooltip absolute w-[400px] bg-black pointer-events-none opacity-90 z-[1000] rounded-[8px]"
						style="left: {tooltipX}px; top: {tooltipY}px;"
					>
						<div
							class="title text-2xl text-[#f0e7e5] text-center bg-cover bg-center w-full inline-block mb-[15px] py-[10px] rounded-[8px]"
							style={`background-image: url('${base}/tooltip-header.png');`}
						>
							{tooltipContent.name}
						</div>
						<div
							class="font-['Fontin SmallCaps', sans-serif] font-light text-[#7d7aad] mb-[15px] px-5"
						>
